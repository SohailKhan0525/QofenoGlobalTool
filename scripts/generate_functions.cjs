...
const functionsDir = path.join(__dirname, 'functions', 'tools');
...
const mainJsTemplate = (name) => `import { Client, Databases, ID, Permission, Role, Storage } from 'node-appwrite';
...
export default async ({ req, res, error }) => {
  ...
  try {
    ...
    const { outputBuffer, outputName } = await processWithRetry(async () => {
      ...
      const outputName = inputName.replace(/\\.[^/.]+$/, "") + '-output.pdf';
...
for (const tool of newTools) {
  const toolDir = path.join(functionsDir, tool);
  if (!fs.existsSync(toolDir)) fs.mkdirSync(toolDir, { recursive: true });
  
  fs.writeFileSync(path.join(toolDir, 'package.json'), packageJsonTemplate(tool));
  
  const srcDir = path.join(toolDir, 'src');
  if (!fs.existsSync(srcDir)) fs.mkdirSync(srcDir, { recursive: true });
  
  fs.writeFileSync(path.join(srcDir, 'main.js'), mainJsTemplate(tool));
}

console.log('Successfully generated boilerplates for new tools.');
