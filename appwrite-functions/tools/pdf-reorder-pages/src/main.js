 ...
 * Input: new_order = "3,1,2,4" (comma-separated 1-based page indices in desired order)
 */
...
export default async ({ req, res, log, error }) => {
  ...
  try {
    ...
    // new_order: "3,1,2,4" or array [3,1,2,4]
    let newOrder = body.new_order;
    if (typeof newOrder === 'string') {
      newOrder = newOrder.split(',').map(s => parseInt(s.trim(), 10)).filter(n => !isNaN(n));
    }
    if (!Array.isArray(newOrder) || newOrder.length === 0) {
      throw new Error('new_order is required (e.g., "3,1,2,4" for 4-page PDF)');
    }
    ...
    const { outputBuffer, outputName } = await processWithRetry(async () => {
      ...
      // Validate order
      const validOrder = newOrder.filter(n => n >= 1 && n <= totalPages);
      if (validOrder.length === 0) throw new Error('No valid page numbers in new_order');

      log(`Reordering ${totalPages} pages to: [${validOrder.join(', ')}]`);

      // Build reordered indices (0-based)
      const indices = validOrder.map(n => n - 1);
      ...
      const outName = inputName.replace(/\.pdf$/i, '') + '-reordered.pdf';
    ...
    await createExecutionRecord(db, {
      user_id: body.user_id || null, tool_slug: 'pdf-reorder-pages', tool_name: 'Reorder PDF Pages',
