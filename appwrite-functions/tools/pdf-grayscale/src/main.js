/**
 * pdf-grayscale — Convert PDF to grayscale.
 * Uses pdf-lib to remove color from PDF elements.
 */
  ...
  try {
    ...
    const { outputBuffer, outputName } = await processWithRetry(async () => {
      ...
      if (isGhostscriptAvailable()) {
        // Ghostscript grayscale conversion — most accurate
        tmpDir = mkdtempSync(join(tmpdir(), 'qofeno-gs-'));
        const inputPath = join(tmpDir, 'input.pdf');
        const outputPath = join(tmpDir, 'output.pdf');
        writeFileSync(inputPath, source.buffer);
        execSync(
          `gs -sOutputFile="${outputPath}" -sDEVICE=pdfwrite -sColorConversionStrategy=Gray -dProcessColorModel=/DeviceGray -dCompatibilityLevel=1.4 -dNOPAUSE -dBATCH -dQUIET "${inputPath}"`,
          { stdio: 'pipe', timeout: 120000 }
        );
        if (!existsSync(outputPath)) throw new Error('Ghostscript grayscale failed');
        outBuf = readFileSync(outputPath);
        log(`Ghostscript grayscale: ${source.buffer.length} → ${outBuf.length} bytes`);
      } else {
        // Fallback: pdf-lib save (doesn't truly convert to grayscale but preserves structure)
        log('Ghostscript not available, using pdf-lib fallback (structure-only)');
        const pdfDoc = await PDFDocument.load(source.buffer, { ignoreEncryption: true });
        outBuf = Buffer.from(await pdfDoc.save({ useObjectStreams: true }));
      }

      const outName = inputName.replace(/\.pdf$/i, '') + '-grayscale.pdf';
    ...
    await createExecutionRecord(db, {
      user_id: body.user_id || null, tool_slug: 'pdf-grayscale', tool_name: 'PDF Grayscale Converter',
    ...
    if (tmpDir) {
      try { ['input.pdf', 'output.pdf'].forEach(f => { const p = join(tmpDir, f); if (existsSync(p)) unlinkSync(p); }); } catch (_) {}
    }
  }
};
