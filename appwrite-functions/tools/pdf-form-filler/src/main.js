...
export default async ({ req, res, log, error }) => {
  ...
  try {
    ...
    const fieldsToFill = body.form_fields || body.fields || {};
      ...
      try {
        for (const [key, value] of Object.entries(fieldsToFill)) {
          ...
          if (field) {
            ...
            else if (fieldType === 'PDFCheckBox') { if(value === true || value === 'true') field.check(); else field.uncheck(); }
            else if (fieldType === 'PDFRadioGroup') field.select(String(value));
            else if (fieldType === 'PDFDropdown') field.select(String(value));
          }
        }
      } catch (err) {
        log(`Warning: some fields could not be filled: ${err.message}`);
      }

      const outBuf = Buffer.from(await pdfDoc.save({ useObjectStreams: true }));
      const outName = inputName.replace(/\.pdf$/i, '') + '-filled.pdf';
    ...
    await createExecutionRecord(db, {
      user_id: body.user_id || null, tool_slug: 'pdf-form-filler', tool_name: 'PDF Form Filler',
