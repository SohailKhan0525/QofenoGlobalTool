...
export default async ({ req, res, log, error }) => {
  ...
  try {
    ...
    const tempOutput = '/tmp/output-' + Date.now() + '.m4r';
    fs.writeFileSync(tempInput, source.buffer);

    await new Promise((resolve, reject) => {
      let command = ffmpeg(tempInput);
      command.audioCodec('aac').setStartTime(0).setDuration(30);
    ...
    const outputName = inputName.replace(/\.[^/.]+$/, '') + '-output.m4r';
    ...
    await createExecution(db, {
      ...
      tool_slug: 'ringtone-maker',
      tool_name: 'ringtone-maker',
