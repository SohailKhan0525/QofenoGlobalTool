...
export default async ({ req, res, log, error }) => {
  ...
  const slug = 'pdf-booklet-creator';
  ...
  try {
    ...
    // Add blank pages to make total page count a multiple of 4
    const originalCount = pdfDoc.getPageCount();
    let pageCount = originalCount;
    while (pageCount % 4 !== 0) {
    ...
    // Assuming uniform page sizes. Grab the first page size
    const { width, height } = pdfDoc.getPage(0).getSize();
    const isPortrait = height > width;
    
    // The new page will have twice the width (or height if landscape)
    const newWidth = isPortrait ? width * 2 : width;
    const newHeight = isPortrait ? height : height * 2;

    const embeddedPages = await newPdf.embedPdf(pdfDoc, pdfDoc.getPageIndices());

    let leftIndex = pageCount - 1;
    let rightIndex = 0;
    
    while (rightIndex < leftIndex) {
      // Front sheet: (N, 1) -> (leftIndex, rightIndex)
      const page1 = newPdf.addPage([newWidth, newHeight]);
      page1.drawPage(embeddedPages[leftIndex], { x: 0, y: 0 });
      page1.drawPage(embeddedPages[rightIndex], { x: width, y: 0 });
      
      leftIndex--;
      rightIndex++;

      // Back sheet: (2, N-1) -> (rightIndex, leftIndex)
      const page2 = newPdf.addPage([newWidth, newHeight]);
      page2.drawPage(embeddedPages[rightIndex], { x: 0, y: 0 });
      page2.drawPage(embeddedPages[leftIndex], { x: width, y: 0 });
