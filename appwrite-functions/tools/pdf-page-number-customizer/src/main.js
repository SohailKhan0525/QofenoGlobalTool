...
export default async ({ req, res, log, error }) => {
  ...
  const slug = 'pdf-page-number-customizer';
  ...
  try {
    ...
    const { outputBuffer, outputName } = await processWithRetry(async () => {
      ...
      const startAt = body.start_at ? parseInt(body.start_at) : 1;
      pages.forEach((page, idx) => {
         page.drawText(String(startAt + idx), { x: page.getWidth() / 2, y: 20, size: 12 });
      });
