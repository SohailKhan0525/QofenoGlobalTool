...
export default async ({ req, res, error }) => {
  ...
  try {
    ...
    const fit = String(body.fit || 'cover');

    let transformer = sharp(source.buffer);
    if (width || height) {
      transformer = transformer.resize(width || null, height || null, {
        fit,
        withoutEnlargement: body.without_enlargement !== false,
      });
    }

    const outputFormat = String(body.output_format || body.format || 'webp').toLowerCase();
    let outputBuffer;
    let outputName;
