...
const LETTER_X_COLLAPSED = [  9,  18,  25,  32,  39] as const;
const SCROLL_END = 100; // px scroll for full collapse
...
function QofenoLogo({
  ...
  // Fetch /qofeno_full.svg directly from public folder
  useEffect(() => {
    if (!showText) return;
    let isMounted = true;
    fetch('/qofeno_full.svg')
      .then(res => res.text())
      .then(svgText => {
        if (!isMounted || !containerRef.current) return;
        containerRef.current.innerHTML = svgText;
        // Make sure SVG scales properly to container
        const svgEl = containerRef.current.querySelector('svg');
        if (svgEl) {
          svgEl.setAttribute('class', 'block w-full h-full');
          svgEl.setAttribute('fill', 'currentColor');
        }
        setSvgLoaded(true);
      })
      .catch(err => {
        console.error("Failed to load /qofeno_full.svg:", err);
      });
  ...
  // Animate letter groups once SVG is loaded
  useEffect(() => {
    if (!showText || !svgLoaded) return;
    ...
    // Set initial entrance state (shifted left by 55 SVG units, opacity 0)
    letters.forEach((g, i) => {
      g.setAttribute('transform', `translate(${LETTER_X_ENTRANCE[i]}, 0)`);
      g.style.opacity = '0';
    });

    // Staggered entrance animation: slide right to natural resting LETTER_X_REST position
    const ctx = gsap.context(() => {
      letters.forEach((g, i) => {
        ...
        gsap.to(g, {
          attr: { transform: `translate(${LETTER_X_REST[i]}, 0)` },
          duration: 0.85,
          ease: 'power3.out',
          delay,
          onComplete: () => {
    ...
    const handleScroll = () => {
      ...
      letters.forEach((g, i) => {
        const currentX = LETTER_X_REST[i] + (LETTER_X_COLLAPSED[i] - LETTER_X_REST[i]) * progress;
        const currentOp = 1 - progress;
        g.setAttribute('transform', `translate(${currentX.toFixed(2)}, 0)`);
        g.style.opacity = currentOp.toFixed(3);
      });
    };
    ...
    return () => {
      ...
      letters.forEach((g, i) => {
        g.setAttribute('transform', `translate(${LETTER_X_REST[i]}, 0)`);
        g.style.opacity = '1';
      });
    };
  }, [showText, svgLoaded]);
